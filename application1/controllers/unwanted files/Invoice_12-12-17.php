...
class Invoice extends CI_Controller {
  ...
  public function __construct()
  {
	...
	{
		$query_update1 =mysql_query("SELECT * FROM invoice_details WHERE status ='1' ORDER BY id DESC LIMIT 1");
		...
		if(date('m')=='04')
		{
			...
			if($old)
			{
				...
				else 
				{
					...
					if($bond_length == 1)
					{
						$new_bond_noo = '10000'.$va;  
					} 
					else if($bond_length == 2)
					{  
						$new_bond_noo = '1000'.$va; 
					}
...
public function get_itemname()
{
  ...
  //     $data['edit']=$this->invoice_model->invoice_edit($id);
  //     $this->load->view('header');
