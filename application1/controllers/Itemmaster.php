...
class Itemmaster extends CI_Controller {
	...
	function addItem(){
		...
		else
		{
			...
			if(@$_POST['itemcode']!="") { $itemcode=$_POST['itemcode']; } else { $itemcode=0; }
			$data=array(
						'date'		=> date('Y-m-d'),
						'itemcode'	=> $itemcode,
						...
						'uom'		=> $_POST['uom'],
						'pattern_material' => $_POST['pattern_material'],
						'casting_weight' => $_POST['casting_weight'],
						'liquid_weight'=>$_POST['liquid_weight'],
						'yield'=>$_POST['yield'],
						'price'		=> $price,
	...
	function editItem($id){
		...
		//echo $person->itemname.'<br>'.stripslashes($person->itemname);exit;
        $this->validation->sid = $id;
		$this->validation->patterncode = $person->patterncode;
		$this->validation->drawingno = $person->drawingno;
		$this->validation->pattern_material = $person->pattern_material;
		$this->validation->casting_weight = $person->casting_weight;
		$this->validation->liquid_weight = $person->liquid_weight;
		$this->validation->yield = $person->yield;
		$this->validation->itemcode = $person->itemcode;
		$this->validation->itemname = htmlspecialchars($person->itemname);
...
// 			$hsnno =$_POST['hsnno'];
			...
			$data=array('uom'=>$uom,'itemname'=>$_POST['itemname'],'price'=>$price,'taxtype'=>$taxtype,'itemcode'=>$itemcode,'sgst'=>$sgst,'igst'=>$igst,'cgst'=>$cgst,'priceType'=>$priceType,'patterncode' => $_POST['patterncode'],'drawingno'=>$_POST['drawingno'],'pattern_material' => $_POST['pattern_material'],'casting_weight' => $_POST['casting_weight'],'liquid_weight'=>$_POST['liquid_weight'],'yield'=>$_POST['yield']);
		//	print_r($data);exit;
	...
	public function ajax_list()
	{
		...
		foreach ($list as $person) {
			...
			@$grade = $this->db->select('grade')->where('id',$person->grade)->get('grade')->row()->grade;
			$no++;
			$row = array();
			$row[] = $i++;
			...
			$row[] = stripslashes($person->itemname);
			$row[] = $person->patterncode;
			$row[] = $uom;
			...
			foreach ($objPHPExcel->getWorksheetIterator() as $sheet)
			{
				...
				for($j=3; $j<=$lastRow; $j++)
				{
					...
					$sgst = $objWorksheet->getCellByColumnAndRow(4,$j)->getValue();
					 ...
					 $taxtype= $objWorksheet->getCellByColumnAndRow(7,$j)->getValue();
					 
					$priceType = $objWorksheet->getCellByColumnAndRow(9,$j)->getValue();
                     
					if($itemname!="")
					{
						$hsnno =  $hsnno ? $hsnno:'0';
						//$itemcode = $itemcode ? $itemcode :'0';
						...
						if($sgst!="" && $cgst!="" && $igst!="")
						{
							$taxdetails=$this->db->where('sgst',$sgst)->where('cgst',$cgst)->where('igst',$igst)->get('vat_details')->row();
						
							if(count(array($taxdetails)) > 0)
							{
								//foreach($taxdetails as $td)
								$taxType=$taxdetails->id;
							}
							else
							{
								$vatdata=array(
									...
									'taxtype'=>'gst',
									'taxname'=>$igst,
									'taxpercentage'=>'gst @ '.$igst.' %',
									'sgst'=>$sgst,
									'cgst'=>$cgst,
									'igst'=>$igst,
									'status'=>1
									);
								$this->db->insert('vat_details',$vatdata);
								$taxType=$this->db->insert_id();
							}
						}
						else
						{
							...
							$getuom=$this->db->where('uom',$uom)->get('uom')->row();
							if(count(array($getuom))>0){
							    $getuomid=$getuom->id;
							}else{
							    $getuomid=$uom;
							}
