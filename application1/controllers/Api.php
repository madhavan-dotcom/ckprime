...
$data['pre']=$this->db->where('id',$id)->get('invoice_details')->result();
	...
	// $point = round($number - $no, 2) * 100;
      ...
      $filename = $id.'.pdf';
      $html = $this->load->view('invoicebill_overalltaxapp',$data,true);
...
//exit;
      ...
      $html = $this->load->view('invoicebill_multipletaxapp',$data,true);
  ...
  public function bill()
  {
    ...
    //   $id = $this->input->post('id');
      $filename = $this->print_pdf($id);

   $url= 'https://my-gst.com/erp_billing/pdf/';
...
$data['fin']=$result;
  ...
  public function print_dc($id)
  {
        ...
        $filename = $id.'dc.pdf';
        $html = $this->load->view('dcbillsapp',$data,true);
...
public function print_quotation($id)
{
    ...
    $data['invoice']=$this->db->where('id',$id)->get('quotation_details')->result();
     ...
     $quotation_tax=$this->db->where('id',1)->get('preference_settings')->row();
		if($quotation_tax->quot_bill_type=="Overall Tax"){
		     $filename = $id.'quot.pdf';
		    $html = $this->load->view('quotationbill_overalltaxapp',$data,true);
		}elseif($quotation_tax->quot_bill_type=="Multiple Tax"){
		     $filename = $id.'quot.pdf';
		    $html = $this->load->view('quotationbillapp',$data,true);
		}
...
public function quotationbill()
  {
   $id=$this->uri->segment(3);
   $filename = $this->print_quotation($id);
     $url= 'https://my-gst.in/erp_billing/pdf/';
...
public function inward_pending()
{
  ...
  if(count($query)  >0)
  {
      foreach($query as $final)
      {
          ...
          $data['id']            = $final['id'];
          $data['inwardno']      = $final['inwardno'];
          $data['inwarddate']    = $final['inwarddate'];
          $data['customername']  = $final['cusname'];
          $data['cusdcno']       = $final['customerdcno'];
          $data['itemname']      = $final['itemname'];
          $data['inwardqty']     = $final['qty'];
          $data['balanceqty']    = $final['balanceqty'];
...
public function voucher_details_receivable()
{
    $query = $this->db->where('status',1)->where('vouchertype','payment')->get('voucher')->result_array();
    if(count($query) > 0)
    {
        foreach($query as $final)
        {
              $data['result']          =   'Success';
              $data['voucherid']       = $final['voucherid'];
              $data['customername']    = $final['name'];
              $data['vouchertype']     = $final['vouchertype'];
              $data['paymentmode']     = $final['paymentmode'];
              $data['paidamount']      = $final['amount'];
              $data['chamount']        = $final['chamount'];
              $data['neft']            = $final['banktransfer'];
...
public function voucher_details_payable()
{
    $query = $this->db->where('status',1)->where('vouchertype','receipt')->get('voucher')->result_array();
    if(count($query) > 0)
    {
        foreach($query as $final)
        {
