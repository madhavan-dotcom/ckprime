...
ob_start();
error_reporting(0);
class OldcashBill extends CI_Controller {

public function __construct()
{
parent::__construct();
$this->load->model('oldcashbill_model');
...
$this->load->view('oldcashbill_list');
...
$data['result']=$this->db->where('id',$id)->get('cashbill_details_backup')->result_array();  
$this->load->view('header');
...
{
$list = $this->oldcashbill_model->get_datatables();
$data = array();
$no = $_POST['start'];
$i=1;
...
<li><a class="" style="color:white; font-weight: bold;background-color: #23BDCF;" href="'.base_url('oldcashbill/viewBill/'.$code).'" title="Hapus" >View</a></li>
<li><a class="" style="color:white; font-weight: bold;background-color: #23BDCF;" target="_blank" href="'.base_url('oldcashbill/printBill/'.$code).'" title="Hapus" >Print</a></li>
</ul>
</div>';
...
$output = array(
"draw" => $_POST['draw'],
"recordsTotal" => $this->oldcashbill_model->count_all(),
"recordsFiltered" => $this->oldcashbill_model->count_filtered(),
"data" => $data,);
...
$this->db->insert('cashbill_party_statement',$dd);
...
$this->db->where('invoiceid',$del)->delete('cashbill_party_statement');
echo'yes';
}
else
{
...
$data['pre']=$this->db->where('id',$id)->get('cashbill_details_backup')->result();
