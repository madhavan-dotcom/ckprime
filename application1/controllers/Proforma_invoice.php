...
class Proforma_invoice extends CI_Controller {
  ...
  public function __construct()
  {
	...
	{
		// $query_update1 =mysql_query("SELECT * FROM get_stateCode WHERE status ='1' ORDER BY id DESC LIMIT 1");
		// while($row = mysql_fetch_array($query_update1))
$query_update1 =$this->db->where('status',1)->order_by('id','desc')->limit(1)->get('proforma_invoice_details')->result_array();
foreach($query_update1 as $row)
	...
	$proforma_type=$this->db->select('proforma_type')->where('id', '1')->get('preference_settings')->row()->proforma_type;
	if($proforma_type=='Overall Tax'){
   ...
   $query=$this->db->where('status',1)->where('cusname',$cusname)->where('balanceqty >',0)->group_by('dcno')->get('dc_delivery');
	...
	public function getdc_details()
	{
		...
		else
		{
          ...
          if($proforma_type=='Overall Tax')
					{
						$this->load->view('proforma_overall',$data);
					}
        elseif($proforma_type=='Multiple Tax'){
