...
class Joborder_dc extends CI_Controller
{
  public function __construct()
  {
    parent::__construct();
    $this->load->model('joborderdc_model');
  ...
  {
    $query_update1 = $this->db->where('status', 1)->order_by('id', 'desc')->limit(1)->get('jobworkdc_details')->result_array();
    foreach ($query_update1 as $row) {
      @$quotationnos = $row['dcno'];
      $default_bond = $this->db->where('id', 1)->get('preference_settings')->row();
      $new_bond_prefix = $default_bond->jobdcPrefix;
    ...
    if (date('m') == '04') {
      $month = date('m');
      $year = date('Y');
      $old = $this->db->where('month(date)', $month)->where('year(date) ', $year)->get('jobworkdc_details')->result_array();
      if ($old) {
        @$bond_no = $quotationnos;
        if (is_null($bond_no)) {
          $default_bond = $this->db->where('id', 1)->get('preference_settings')->row();
          $new_bond_prefix = $default_bond->jobdcPrefix;
          $new_bond_noo = $new_bond_prefix . $default_bond->jobworkdc;
          //$new_bond_noo = 'D00001'; 
        } else {
          $default_bond = $this->db->where('id', 1)->where('jobworkdc !=', '')->get('preference_settings')->row();

          if ($default_bond) {
            $bond_no = $default_bond->jobworkdc;
    ...
    $this->load->view('add_joborderdc', $data);
  ...
  // 'delete_status'=>1
  // );
  // $result=$this->joborderdc_model->add($data);
  // if($result==true)
  // {
  // $insertids=$this->db->insert_id();
  // $this->db->query("UPDATE preference_settings SET jobworkdc='' WHERE id=1");
  // $hsnnos=$_POST['hsnno'];
  ...
  // 'dc_status'=>1, 
  // );
  // $this->db->insert('jobworkdc_delivery',$datas1);
  // }
  ...
  // if($_POST['save'])
  // {
  // redirect('joborder_dc');
  // }
  // if($_POST['print'])
  // {
  // redirect('joborder_dc/bill');
  // }
  // }
  // else
  // {
  ...
  public function insert()
  {
    ...
    $data = array(
      ...
      'cusname'       => $this->input->post('customername'),
      'customerId'    => $this->input->post('customerId'),
      'vehicleno'     => $this->input->post('vehicleno'),
      'time'          => $this->input->post('time'),
      'purpose'       => $this->input->post('purpose'),
      'process'       => $this->input->post('process'),
      'remarkss'      => $this->input->post('remarkss'),
      'address'       => $this->input->post('address'),
      'itemname'      => implode('||', $this->input->post('itemname')),
      'itemid'        => implode('||', $this->input->post('itemid')),
      'item_desc'     => implode('||', $this->input->post('item_desc')),
      'qty'           => implode('||', $this->input->post('qty')),
      'weight'        => implode('||', $this->input->post('weight')),
      'hsnno'         => implode('||', $this->input->post('hsnno')),
      'itemcode'      => implode('||', $this->input->post('itemcode')),
      'uom'           => implode('||', $this->input->post('uom')),
    ...
    $result = $this->db->insert('jobworkdc_details', $data);
    if ($result == true) {
      $insertids = $this->db->insert_id();
      $this->db->query("UPDATE preference_settings SET jobworkdc='' WHERE id=1");
      $hsnnos = $_POST['hsnno'];
      ...
      $this->session->set_flashdata('msg', 'Joborder Dc Added Successfully');
      if ($_POST['save']) {
        redirect('joborder_dc');
      }
      if ($_POST['print']) {
      ...
      $this->session->set_flashdata('msg1', 'Joborder Dc Added Unsuccessfully');
      redirect('joborder_dc');
    }
  }
  ...
  {
    $data['pre'] = $this->db->where('status', 1)->order_by('id', 'desc')->limit(1)->get('jobworkdc_details')->result();
  ...
  public function rebill()
  {
    ...
    $data['pre'] = $this->db->where('id', $id)->get('jobworkdc_details')->result();
    $this->load->view('dcbills', $data);
  }

  public function view()
  {
    $data['dcbill'] = $this->joborderdc_model->select();
    $this->load->view('header');
    $this->load->view('joborderdc_view', $data);
  ...
  {
    $data['view'] = $this->joborderdc_model->select_pending();
    $this->load->view('header');
    $this->load->view('jobworkdc_pending', $data);
    $this->load->view('footer');
  }
  ...
  public function edit()
  {
    ...
    $data['result'] = $this->db->where('id', $id)->get('jobworkdc_details')->result_array();
    $this->load->view('header');
    $this->load->view('edit_joborderdc', $data);
    $this->load->view('footer');
  }
  ...
  public function update()
  {
    ...
    $this->db->where('id', $id);
    $result = $this->db->update('jobworkdc_details', $data);
    if ($result == true) {
      $check = $this->db->where('insertid', $id)->get('jobworkdc_delivery')->result_array();
      if ($check) {
        $delcount = count($check);
        for ($m = 0; $m < $delcount; $m++) {
          $this->db->where('insertid', $id);
          $this->db->delete('jobworkdc_delivery');
        }
      }
  ...
  {
    $list = $this->joborderdc_model->get_datatables();
    $data = array();
    $no = $_POST['start'];
    $a = 1;
    $totalamount[] = array();
...
<li><a class="" target="_blank" style="color:white; font-weight: bold;background-color: #23BDCF;" href="' . base_url() . 'joborder_dc/rebill/' . $person->id . '" title="Print" >Print</a></li>
';
      $delestatus = $person->edit_status;
      if ($delestatus == 1) {
        $editLink = base_url() . 'joborder_dc/edit/' . base64_encode($person->id);
...
else
{
    ...
    $output = array(
      "draw" => $_POST['draw'],
      "recordsTotal" => $this->joborderdc_model->count_all(),
      "recordsFiltered" => $this->joborderdc_model->count_filtered(),
      "data" => $data,
    );
  ...
  public function search_reports()
  {
    ...
    if ($bill_format == 'Print') {
      $data['purchase'] = $this->joborderdc_model->search_billing();
      ...
      $this->load->view('joborderdc_reports', $data);
    }
  }
  ...
  {
    $id = $this->input->post('id');
    $data = $this->db->where('id', $id)->get('jobworkdc_details')->result_array();
    if ($data) {
      foreach ($data as $row) {
        $html = '<div class="row">
...
<div class="col-md-4">
  ...
  {
    $data = $this->db->where('id', $_POST['id'])->delete('jobworkdc_details');
    if ($data) {
      $checkdata = $this->db->where('insertid', $_POST['id'])->get('jobworkdc_delivery')->result_array();
      if ($checkdata) {
      ...
      for ($i = 0; $i < $count; $i++) {
        $this->db->where('insertid', $_POST['id'])->delete('jobworkdc_delivery');
      }
  ...
  public function get_name()
  {
    ...
    $this->db->select('*');
    $this->db->from('customer_details');
    $this->db->where("(type = 'Subcontract')");
  ...
  {
    $name = $_POST['cusname'];
    $data = $this->db->where("(type = 'Subcontract')")->where('name', $name)->get('customer_details')->result();
    echo $count = count($data);
  }
...
$(this).val(val);
  ...
  {
    $id = $this->input->post('id');
    $data = $this->db->where('insertid', $id)->get('jobworkdc_delivery')->result_array();
