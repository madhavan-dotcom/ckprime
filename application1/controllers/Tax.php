...
class Tax extends CI_Controller {
	...
	public function hsnexcel_download()
	{
		$fromdate=$this->input->post('fromdate');
		$todate=$this->input->post('todate');
		$cusname=$this->input->post('cusname');
		...
		$data=array(
			...
			'rcbio_gsttype' => $gsttype,
			'rcbio_tax_percent' => $tax_percent,
			'rcbio_bill_format' =>'hsnexceldownload',
			);
	...
	{   
		...
		if($bill_format=='Print')
		{
			...
			$this->load->view('taxinvoice_reports',$data);
		}
		...
		$exceldata=array();
			foreach ($result as $row)
			{
				...
				{
					$amount=explode('||',$row['taxableamount']);	  
		...
		elseif($bill_format =="hsnexceldownload")
		{
			$result = $this->taxstatement_model->search_billing();
	
			$this->load->library('excel');
			$this->excel->setActiveSheetIndex(0);
			$this->excel->getActiveSheet()->setTitle('Tax Report with Hsn No');
	
			$this->excel->getActiveSheet()->setCellValue('A1','Tax Report with Hsn No');
		
			$this->excel->getActiveSheet()->setCellValue('A2','Date');
			$this->excel->getActiveSheet()->setCellValue('B2','Invoice NO');
			$this->excel->getActiveSheet()->setCellValue('C2','Customer Name');
			$this->excel->getActiveSheet()->setCellValue('D2','GST No');
			$this->excel->getActiveSheet()->setCellValue('E2','HSN No');
			$this->excel->getActiveSheet()->setCellValue('F2','Item Name');
			$this->excel->getActiveSheet()->setCellValue('G2','Qty');
			$this->excel->getActiveSheet()->setCellValue('H2','Basic Amount');
			$this->excel->getActiveSheet()->setCellValue('I2','Tax Type');
			$this->excel->getActiveSheet()->setCellValue('J2','CGST Percentage');
			$this->excel->getActiveSheet()->setCellValue('K2','CGST Amount');
			$this->excel->getActiveSheet()->setCellValue('L2','SGST Percentage');
			$this->excel->getActiveSheet()->setCellValue('M2','SGST Amount');
			$this->excel->getActiveSheet()->setCellValue('N2','IGST Percentage');
			$this->excel->getActiveSheet()->setCellValue('O2','IGST Amount');
			$this->excel->getActiveSheet()->setCellValue('P2','Total Amount');
		   ...
		   else
		   {
			...
			//   print_r($exceldata);die;
			  ...
			  $filename='Invoice HSN Tax Report-'.date('d/m/y').'.xls'; //save our workbook as this file name
			  header('Content-Type: application/vnd.ms-excel'); //mime type
			  header('Content-Disposition: attachment;filename="'.$filename.'"'); //tell browser what's the file name
