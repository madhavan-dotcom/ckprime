...
ob_start();
class Material_dc extends CI_Controller {
public function __construct()
{
parent::__construct();
$this->load->model('materialdc_model');
...
{
// $query_update1 =mysql_query("SELECT * FROM materialdc_details WHERE status ='1' ORDER BY id DESC LIMIT 1");
// while($row = mysql_fetch_array($query_update1))  

$query_update1=$this->db->where('status',1)->order_by('id','desc')->limit(1)->get('materialdc_details')->result_array();
foreach($query_update1 as $row)
{
@$quotationnos=$row['dcno'];  
$default_bond=$this->db->where('id',1)->get('preference_settings')->row();
$new_bond_prefix = $default_bond->materialdcPrefix;
...
$month=date('m');
$year=date('Y');
$old=$this->db->where('month(date)', $month)->where('year(date) ', $year)->get('materialdc_details')->result_array();
if($old)
{
...
$default_bond=$this->db->where('id',1)->get('preference_settings')->row();
$new_bond_prefix = $default_bond->materialdcPrefix;
$new_bond_noo = $new_bond_prefix.$default_bond->materialdc;
//$new_bond_noo = 'D00001'; 
} 
else 
{
$default_bond=$this->db->where('id',1)->where('materialdc !=','')->get('preference_settings')->row();
if($default_bond)
{
$bond_no=$default_bond->materialdc;
...
$this->load->view('add_materialdc',$data);
...
$result=$this->materialdc_model->add($data);
if($result==true)
{
$insertids=$this->db->insert_id();
$this->db->query("UPDATE preference_settings SET materialdc='' WHERE id=1");
$hsnnos=$_POST['hsnno'];
...
$this->db->insert('materialdc_delivery',$datas1);
}
...
if($_POST['save'])
{
redirect('material_dc');
}
if($_POST['print'])
{
redirect('material_dc/bill');
}
}
else
{
...
{
$data['pre']=$this->db->where('status',1)->order_by('id','desc')->limit(1)->get('materialdc_details')->result();
...
$data['pre']=$this->db->where('id',$id)->get('materialdc_details')->result();
$this->load->view('materaildcbills',$data);
}

public function view()
{
$data['dcbill']=$this->materialdc_model->select();
$this->load->view('header');
$this->load->view('materialdc_view',$data);
...
{
$data['view']=$this->materialdc_model->select_pending();
$this->load->view('header');
...
$data['result']=$this->db->where('id',$id)->get('materialdc_details')->result_array(); 
$this->load->view('header');
$this->load->view('edit_materialdc',$data);
$this->load->view('footer');
}
...
$result=$this->materialdc_model->update($data,$id);
if($result==true)
{
$check=$this->db->where('insertid',$id)->get('materialdc_delivery')->result_array();
if($check)
{
...
$this->db->delete('materialdc_delivery');
}
}
...
{
$list = $this->materialdc_model->get_datatables();
$data = array();
$no = $_POST['start'];
$a=1;
$totalamount[]=array();
...
<li><a class="" target="_blank" style="color:white; font-weight: bold;background-color: #23BDCF;" href="'.base_url().'material_dc/rebill/'.$person->id.'" title="Print" >Print</a></li>
';
$delestatus=$person->delete_status;
if($delestatus==1) 
{ 
$editLink = base_url().'material_dc/edit/'.base64_encode($person->id);
...
$output = array(
"draw" => $_POST['draw'],
"recordsTotal" => $this->materialdc_model->count_all(),
"recordsFiltered" => $this->materialdc_model->count_filtered(),
"data" => $data,
);
...
{
$data['purchase']=$this->materialdc_model->search_billing();
...
{
$id=$this->input->post('id');
$data=$this->db->where('id',$id)->get('materialdc_details')->result_array();
if($data)
{
foreach ($data as $row) {
$html='<div class="row">
...
{    
$data=$this->db->where('id',$_POST['id'])->delete('materialdc_details');
if($data)
{
$checkdata=$this->db->where('insertid',$_POST['id'])->get('materialdc_delivery')->result_array();
if($checkdata)
{
...
for ($i=0; $i <$count ; $i++) { 
$this->db->where('insertid',$_POST['id'])->delete('materialdc_delivery');
}
...
{
$this->load->view('materialdirectdc');
}
}
...
{
$id=$this->input->post('id');
$data=$this->db->where('insertid',$id)->get('materialdc_delivery')->result_array();
...
$id=$this->input->post('id');
$data['result']=$this->db->where('id',$id)->get('materialdc_details')->result_array();
$this->load->view('editmateraildc',$data);
}
}
