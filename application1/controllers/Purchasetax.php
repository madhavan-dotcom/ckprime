...
ob_start();
error_reporting(0);
class Purchasetax extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model('purchasetaxstatement_model');
	...
	public function index()
	{
        ...
        $data['invoice']=$this->purchasetaxstatement_model->select();
		$this->load->view('header');
		$this->load->view('purchasetax_reports',$data);
	...
	{   
		...
		{
			$data['invoice']=$this->purchasetaxstatement_model->search_billing();
			...
			$data['purchaseno']=$this->session->userdata('rcbio_purchaseno');
			$data['gsttype']=$this->session->userdata('rcbio_gsttype');
			$data['tax_percent']=$this->session->userdata('rcbio_tax_percent');
			$this->load->view('purchase_reportss',$data);
		}
		elseif($bill_format=='download')
		{
			$result = $this->purchasetaxstatement_model->search_billing();
			...
			{
				 @$gstin=$this->db->select('gstno')->where('id',$row['supplierId'])->where("(type = 'Intra supplier' OR type = 'Inter supplier')")->get('customer_details')->row()->gstno;
				 if($row['amount']!="")
				{
				...
				$data['purchasedate']=date('d-m-Y',strtotime($row['purchasedate']));
				$data['invoiceNo']=' '.$row['invoiceno'];
				$data['Company']=$row['suppliername'];
				$data['gstin']=$gstin;
			...
			$filename='Purchase Tax Report-'.date('d/m/y').'.xls'; //save our workbook as this file name
  ...
  {
    $list = $this->purchasetaxstatement_model->get_datatables();
    $data = array();
    $no = $_POST['start'];
    $i=1;
    foreach ($list as $person) {

       @$gstin=$this->db->select('gstno')->where('id',$person->supplierId)->where("(type = 'Intra supplier' OR type = 'Inter supplier')")->get('customer_details')->row()->gstno;

       @$phoneno=$this->db->select('phoneno')->where('id',$person->supplierId)->where("(type = 'Intra supplier' OR type = 'Inter supplier')")->get('customer_details')->row()->phoneno;
...
$output = array(
  "draw" => $_POST['draw'],
  "recordsTotal" => $this->purchasetaxstatement_model->count_all(),
  "recordsFiltered" => $this->purchasetaxstatement_model->count_filtered(),
  "data" => $data,);
