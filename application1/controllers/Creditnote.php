...
class Creditnote extends CI_Controller {
  ...
  public function pay()
  {
    ...
    $this->db->limit(1);
    $num=$this->db->get('creditnote')->result_array();

    @$invoice=$num[0]['creditnoteid'];
    $count=count($invoice);
    if($count=='0')
    {
      $gg="CR00001";
    ...
    $data['pay']=$this->creditnote_model->pay_amount($id);
  ...
  { 
    ...
    {
		$default_bond=$this->db->where('id',1)->where('creditnote !=','')->get('preference_settings')->row();
		if($default_bond)
		{
			@$bond_no='CR'.$default_bond->creditnote;
			$bondLen = strlen($bond_no)-1;
			$bondOnlyNum = filter_var($bond_no, FILTER_SANITIZE_NUMBER_INT);
			$sales_no = 'CR'.sprintf('%0'.$bondLen.'d', $bondOnlyNum);
		}
		else
		{
			$bondLen = strlen($cusid)-1;
			$bondOnlyNum = filter_var($cusid, FILTER_SANITIZE_NUMBER_INT);
			$sales_no = 'CR'.sprintf('%0'.$bondLen.'d', $bondOnlyNum + 1);
		}
      
    }
    $data['creditnoteid']=$sales_no;
	...
	Public function insert()
	{
		...
		if($_POST['vouchertype']=='payment')
		{
			...
			@$invoiceamt='-';
			$da = array(
			'date'=>date('Y-m-d',strtotime($_POST['creditnotedate'])),
			'receiptdate'=>date('Y-m-d',strtotime($_POST['creditnotedate'])),
			'invoiceno'=>$_POST['invoiceno'],
			'receiptno'=>$creditnoteid,
			'customername'=>$_POST['name'],
			'returnamount'=>$_POST['debitAmount'],
			'status'=>1
			);      
		...
		'creditnotedate'=>date('Y-m-d',strtotime($_POST['voucherdate'])),
    'debitAmount'=>$_POST['debitAmount'],
		'status'=>1
		);  
		$result=$this->db->insert('creditnote',$data);
		if($result==true)
		{
			$this->db->query("UPDATE preference_settings SET creditnote='' WHERE id=1");
			$this->session->set_flashdata('msg','creditnote Added Successfully');
					...
					else
			{
				...
				redirect('creditnote/bill');
			}
		}
		else
		{
			$this->session->set_flashdata('msg1','creditnote Added Failed');
			redirect('creditnote');
		}
	}
 ...
 Public function update()
  {
    ...
    {
      $paymentdetails=$payment.' '.$_POST['cardtype'];
    } 
...
// Supplier Pay The voucher Amount Function
  else
  {
    ...
    $data=array(
      ...
      $this->db->where('id',$id);
      $result=$this->db->update('creditnote',$data);
      if($result==true)
      {
            $this->session->set_flashdata('msg','creditnote update Successfully');
                 redirect('creditnote/reports');
...
// $point = round($number - $no, 2) * 100;
  ...
  {
    $data['voucher']=$this->creditnote_model->select();;
    $this->load->view('header');
  ...
  {
    $list = $this->creditnote_model->get_datatables();
    $data = array();
    $no = $_POST['start'];
    $a=1;
    $totalamount[]=array();
...
// $totalamount[]=$person->totalamount;
      ...
      $row[] =$person->creditnoteid;
      $row[] =$person->name;
      
      $row[] = $person->debitAmount;
      ...
      $row[] = '<div class="btn-group">
          ...
          <li><a class="" style="color:white; font-weight: bold;background-color: #23BDCF;" target="_blank" href="'.base_url('creditnote/reprint/'.$person->id).'" title="Hapus" >Print</a></li>
          <li><a class="" style="color:white; font-weight: bold;background-color: #23BDCF;" href="'.base_url('creditnote/edit/'.base64_encode($person->id)).'">Edit</a></li>
    ...
    $output = array(
      "draw" => $_POST['draw'],
      "recordsTotal" => $this->creditnote_model->count_all(),
      "recordsFiltered" => $this->creditnote_model->count_filtered(),
      "data" => $data,
      );
...
//$cusname='ram';
  ...
  {   
    ...
    {
      $data['purchase']=$this->creditnote_model->search_billing();
    ...
    elseif($bill_format=='Bill_Download')
    {
      ...
      $this->load->library('mpdf');
      $purchase=$this->creditnote_model->search_billing();
...
//   $del=base64_decode('Ng==');
   ...
   //$del=$this->input->post('id');  
  $getdetails=$this->db->where('id',$del)->get('creditnote')->result_array();

      if($getdetails)
      {
        foreach ($getdetails as $rows) 
        {
          $creditnoteid=$rows['creditnoteid'];
          $name=$rows['name'];
        ...
        if($vouchertype=='payment')
        {
            ...
            $this->db->where('receiptno',$creditnoteid);
  ...
  $data=$this->db->where('id',$del)->delete('creditnote');
       if($data)
       {
