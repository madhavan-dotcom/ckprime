...
class CashBill extends CI_Controller {
  ...
  public function __construct()
  {
	...
	{
	$query_update1 =$this->db->where('status',1)->order_by('id','desc')->limit(1)->get('cashbill_details')->result_array();
foreach($query_update1 as $row)
		{
			$quotationnos=$row['invoiceno'];
			$default_bond=$this->db->where('id',1)->get('preference_settings')->row();
			$new_bond_prefix = $default_bond->cashbillPrefix;
	...
	public function insert()
	{
		...
		//$result=$this->db->insert('customer_details',$customerDet);
		//if($result==true)
		//{ 
