...
class Olddcbill_model extends CI_Model {




	        var $table = 'dcbill_details_backup';
...
return $query=$this->db->get('dcbill_details_backup')->result_array();
...
return $query->result_array();
	...
	{
          $this->db->insert('dcbill_details_backup',$data);
  ...
  {       
          $this->db->where('id',$id);
          $this->db->update('dcbill_details_backup',$data);
          return $this->db->affected_rows()!=1? false:true;
  }
	...
	{
		$this->db->select('*');
		$this->db->from('dcbill_details_backup');
