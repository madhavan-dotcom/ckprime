...
class Old_postmt_model extends CI_Model {


	var $table = 'po_party_statements_backup';
	var $column_order = array(null, 'date','invoiceno','receiptno','suppliername','purchaseamt','receiptamt','balance','paid'); //set column field database for datatable orderable
	var $column_search = array('invoicedate','invoiceno','receiptno','suppliername','purchaseamt','receiptamt','balance','paid'); //set column field database for datatable searchable 
	...
	{
		$this->db->select('*');
		$this->db->from('po_party_statements_backup');
	...
	public function search_billing()
	{
		...
		//$this->db->where('purchasetype','Direct Purchase');
		return $query=$this->db->get('po_party_statements_backup')->result_array();
