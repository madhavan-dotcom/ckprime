			...
			class Inwardstock_model extends CI_Model {


			var $table = 'inward_delivery';
			var $column = array('itemname','qty','balanceqty','id',); //set column field database for order and search
			var $column_search = array('date','itemname','qty','balanceqty','id'); //set column field database for datatable searchable 
			...
			{
				$this->db->insert('inward_delivery',$data);
			...
			{
				$this->db->insert('inward_delivery',$c);
				return ($this->db->affected_rows() !=1) ? false:true;
			...
			public function check($b)
			{
				...
				$query = $this->db->get('inward_delivery');
				if($query->num_rows >0)
				{
					return true;
				}		
			...
			{	
				...
				$this->db->update('inward_delivery',$data);
				return ($this->db->affected_rows() !=1) ? false:true;
			...
			public function search_stock()
			{
				...
				if($this->input->post('itemname')){$this->db->where('itemname',$this->input->post('itemname'));}
