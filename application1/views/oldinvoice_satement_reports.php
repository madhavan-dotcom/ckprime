...
<?php /* print_r($this->input->post()); exit; */ ?>
	...
	<table width="1000" border="0" align="center" style="border-collapse:collapse; font-family: Arial, Helvetica, sans-serif; font-size: 15px;">
		...
		if($customername)
		{
			...
			$openingbalance=$this->db->select('old_openingBal')->where('name',$customername)->where("(type = 'Inter customer' OR type = 'Intra customer')")->get('customer_details')->row()->old_openingBal;
	...
	//	echo '<br>'.$ret;
		...
		<tr>
			...
			<td align="right" style="border-top:1px solid black; padding:5px;"><strong><?php if($openingbalance!='') { echo number_format($openingbalance,2); } else { echo '0.00'; } ?></strong></td>
		</tr>
