...
$count = count($pono);
for ($i = 0; $i < $count; $i++) {
	$data[] = $this->db->where('purchaseorderno', $pono[$i])->where('balaceqty >', 0)->get('purchaseorder_reports')->result_array();
	...
	<table class="table">
		...
		<tbody>
			...
			?>
					<tr>
						<td>
							<input class="form-control" parsley-trigger="change" readonly id="purchaseorderno<?php echo $rows['id']; ?>" type="text" name="purchaseorderno[]" value="<?php echo $rows['purchaseorderno']; ?>" readonly>
							<input class="form-control" parsley-trigger="change" readonly id="purchaseorder<?php echo $rows['id']; ?>" type="text" name="purchaseorder[]" value="<?php echo $rows['purchaseorder']; ?>" readonly>
							<input class="form-control" parsley-trigger="change" readonly id="id<?php echo $rows['id']; ?>" type="text" name="workorderid[]" value="<?php echo $rows['id']; ?>"readonly>
						...
						<td><input class="itemcode_class" id="itemcode<?php echo $rows['id']; ?>" parsley-trigger="change" readonly style="width:120px;border:1px solid #605f5f;" type="text" name="itemcode[]" value="<?php echo $rows['itemcode']; ?>" style="width:70px;">
								<input type="hidden" name="itemid[]" id="itemid<?php echo $rows['id']; ?>" class="form-control" value="<?php echo $rows['itemid']; ?>" style="width:70px;">
							<div id="itemcode_valid"></div>
						</td>

					

						<td><input class="itemname_class" id="itemname<?php echo $rows['id']; ?>" parsley-trigger="change" readonly style="width:250px;border:1px solid #605f5f;" type="text" name="itemname[]" value="<?php echo htmlspecialchars($rows['itemname']); ?>">
							<div id="itemname_valid"></div><input type="text" name="item_desc[]" value="" id="item_desc<?php echo $rows['id']; ?>" style="width:250px;border:1px solid #605f5f;margin-top: 2px;">
						</td>

						<td><input class="" id="qty<?php echo $rows['id']; ?>" parsley-trigger="change" type="text" name="qty[]" autocomplete="off" style="width:80px;border:1px solid #605f5f;">
							<input class="" type="hidden" id="balanceqty<?php echo $rows['id']; ?>" value="<?php echo $rows['balaceqty']; ?>" parsley-trigger="change" type="text" name="balanceqty[]" autocomplete="off" style="width:80px;border:1px solid #605f5f;">
							<!-- <input class="" type="text" id="balance<?php echo $rows['id']; ?>" data-id="<?php echo $rows['id']; ?>" type="text" name="balance[]"> -->
							<div id="qty_valid<?php echo $rows['id']; ?>" style="color:green;">Qty : <?php echo @$rows['balaceqty']; ?></div>
							<div id="stockqty_valid<?php echo $rows['id']; ?>"></div>
						</td>

						<td><input class="weight_class decimals" data-id="<?php echo $rows['id']; ?>" id="weight<?php echo $rows['id']; ?>" parsley-trigger="change" required type="text" value="<?php echo $rows['weight']; ?>" name="weight[]" autocomplete="off"></td>

						<td><input class="" id="uom" parsley-trigger="change" required readonly style="width:80px;border:1px solid #605f5f;" type="text" name="uom[]" value="<?php echo @$rows['uom']; ?>" autocomplete="off">
							<div id="rate_valid"></div>
						</td>

						<td>
							<select name="grade[]" id="grade<?php echo $rows['id']; ?>" data-id="<?php echo $rows['id']; ?>" class="form-control grade_class" style="width: 120px;">
								<option value="">Select Grade</option>
								<?php
								$grade = $this->db->where('status', 1)->get('grade')->result();
								foreach ($grade as $g) { ?>
									<option value="<?php echo $g->id; ?>"
										<?php echo ($g->id == $rows['grade']) ? 'selected' : ''; ?>>
										<?php echo $g->grade; ?>
									</option>
								<?php } ?>
							</select>
...
<div class="col-sm-offset-4">
	<button class="btn btn-info" id="submit" type="submit" name="save" value="save">Add Invoice</button>
...
?>
		<script type="text/javascript">
			$(document).ready(function() {
				...
				var pos = $('#purchaseorder<?php echo $rows['id']; ?>').val();
				var itemcode = $('#itemcode<?php echo $rows['id']; ?>').val();
				var itemid = $('#itemid<?php echo $rows['id']; ?>').val();
				
			

				
$.post('<?php echo base_url(); ?>invoice/get_heatno_purchase', {
    pos: pos,
    itemcode: itemcode,
	itemid: itemid
}, function(rest) {
    var data = JSON.parse(rest);
    var descriptionText = "heatno : ";

    $.each(data, function(index, item) {
        descriptionText += item.heatno + "-" + item.qty;

        if (index < data.length - 1) {
            descriptionText += " , "; // separator between items
        }
    });

    $('#item_desc<?php echo $rows['id']; ?>').val(descriptionText);
});
				...
				$('#qty<?php echo $rows['id']; ?>').keyup(function() {
					...
					if (qtys > balanceqty) {
						alert('The entered quantity exceeds the balance quantity of ' + balanceqty);
						$('#qty<?php echo $rows['id']; ?>').val(balanceqty);
					} else {
						calculations(totals);
					}
				});
...
<script type="text/javascript">
	...
	$('#document').ready(function() {
		...
		$(".heatno_class").change(function() {
			var rowId = $(this).data("id"); // row ID
			var heatno = $(this).val(); // selected heatno
			...
			$.post("<?php echo base_url(); ?>invoice/get_heatno", {
				...
				if (obj.balance !== undefined) {
					$("#balance" + rowId).val(obj.balance);
					$("#qty" + rowId).val(obj.balance);
				} else {
					$("#qty" + rowId).val('');
				}
