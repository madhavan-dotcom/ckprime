...
?>
	...
	<table width="90%" border="0" align="center" style="border-collapse:collapse; font-family: Arial, Helvetica, sans-serif; font-size: 15px;">
		...
		<?php
		$custId_query=$this->db->query("SELECT customerId,customername FROM invoice_details_backup GROUP BY customerId");
		if($custId_query->num_rows() > 0 )
		{
			...
			foreach($custIdRes as $cR)
			{
				...
				//OPENING BALANCE
				$openingAmt = $this->db->query("SELECT old_openingBal FROM `customer_details` WHERE `id`='".$cR->customerId."' ");
				if($openingAmt->num_rows() > 0 )
				{
					$openingAmtRes = $openingAmt->row();
					$openingBala   = $openingAmtRes->old_openingBal;
				...
				//GET INVOICE AMOUNT	
				$grandInvoiceAmt = $this->db->query("SELECT SUM(`grandtotal`) AS invoiceAmt FROM `invoice_details_backup` WHERE `customerId`='".$cR->customerId."' ");
				...
				//GET RECEIPT AMOUNT
				$grandReceiptAmt = $this->db->query("SELECT SUM(`voucheramount`) AS receiptAmt FROM `voucher_backup` WHERE vouchertype='payment' AND `cus_suppId`='".$cR->customerId."' ");
