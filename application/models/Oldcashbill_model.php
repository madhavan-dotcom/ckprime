...
class Oldcashbill_model extends CI_Model {


        var $table = 'cashbill_details_backup';
var $column_search = array('invoicedate','invoiceno','customername','grandtotal','paid','typesgst','typecgst','typeigst','id'); //set column field database for order and search
  ...
  public function count_all()
  {
    ...
    return $this->db->count_all_results();
		...
		public function search_billing()
		{
			...
			if($this->session->userdata('rcbio_fromdate')!="") { $fromdate=date('Y-m-d',strtotime($this->session->userdata('rcbio_fromdate'))); }  else { $fromdate=''; }
			
			if($this->session->userdata('rcbio_todate')!="") { $todate=date('Y-m-d',strtotime($this->session->userdata('rcbio_todate'))); } else { $todate=''; }
			...
			$gsttype=$this->session->userdata('rcbio_gsttype');
			if($gsttype=='sgst') { $this->db->where('typesgst',$this->session->userdata('rcbio_gsttype')); }
			if($gsttype=='cgst') { $this->db->where('typecgst',$this->session->userdata('rcbio_gsttype')); }
			if($gsttype=='igst') { $this->db->where('typeigst',$this->session->userdata('rcbio_gsttype')); }

			if(@$this->session->userdata('rcbio_invoiceno')) { $this->db->where('invoiceno',$this->session->userdata('rcbio_invoiceno')); }
			return $query=$this->db->get('cashbill_details_backup')->result_array();
		}
...
public function delete_by_id($id)
{
    ...
    {
        $this->db->insert('cashbill_details_backup',$data);
        return $this->db->affected_rows()!=1? false:true;
    }
    ...
    {
        $this->db->select('*');
        $this->db->from('cashbill_details_backup');
        ...
        public function search_invoice()
    {
        ...
        return $query= $this->db->get('cashbill_details')->result_array();
    }
    ...
    public function update_invoice($data,$id)
    {
        ...
        $this->db->where('id',$id);
        $this->db->update('cashbill_details_backup',$data);
        return $this->db->affected_rows()!=1? false:true;
    }
       ...
       public function search_pending()
    {
        ...
        return $query= $this->db->where('balance >',0)->get('cashbill_details_backup')->result_array();
