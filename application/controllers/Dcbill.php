...
{
$query_update1 =$this->db->where('status',1)->order_by('id','desc')->limit(1)->get('dcbill_details')->result_array();
foreach($query_update1 as $row)
{
@$quotationnos=$row['dcno'];  
$default_bond=$this->db->where('id',1)->get('preference_settings')->row();
$new_bond_prefix = $default_bond->salesdcPrefix;
...
$month=date('m');
$year=date('Y');
$old=$this->db->where('month(date)', $month)->where('year(date) ', $year)->get('dcbill_details')->result_array();
if($old)
{
...
$default_bond=$this->db->where('id',1)->get('preference_settings')->row();
$new_bond_prefix = $default_bond->salesdcPrefix;
$new_bond_noo = $new_bond_prefix.$default_bond->dc;
//$new_bond_noo = 'D00001'; 
} 
else 
{
...
$itemcode = is_array($_POST['itemcode'] ?? null) ? implode('||', $_POST['itemcode']) : ($_POST['itemcode'] ?? '');

$uom=implode('||',$_POST['uom']);
$grade=implode('||',$_POST['grade']);
$remarks=implode('||',$_POST['remarks']);
@$inwardno = is_array($_POST['inwardno'] ?? null) ? implode('||', $_POST['inwardno']) : ($_POST['inwardno'] ?? '');

@$customerdcno = is_array($_POST['customerdcno'] ?? null) ? implode('||', $_POST['customerdcno']) : ($_POST['customerdcno'] ?? '');
@$customerdcdate = is_array($_POST['customerdcdate'] ?? null) ? implode('||', $_POST['customerdcdate']) : ($_POST['customerdcdate'] ?? '');
...
'remarkss'=>$_POST['remarkss'],
'approximate_value'=>$_POST['approximate_value'],
'address'=>$_POST['address'], 
'inwardno'=>$inwardno,
// 'customerdcno'=>$customerdcno,
// 'customerdcdate'=>$customerdcdate,edit()
'itemname'=>$itemname,
// 'heatno'=>$heatno,
'itemid'=>$itemid,
'item_desc'=>$item_desc,
...
$itemnamess=$this->input->post('itemname');
...
@$inwardnos = is_array($_POST['inwardno'] ?? null) ? implode('||', $_POST['inwardno']) : ($_POST['inwardno'] ?? '');
...
public function inwards(){
 ...
 $data['inward'] = $this->db->where('inwardno',$inwardno)->where('status',1)->order_by('id','desc')->get('dcbill_details')->result();
 
 
   $this->load->view('header');
 $this->load->view('dcinward_reports',$data);
 $this->load->view('footer1');
}
...
public function excel_download(){
    ...
    $val=$this->dcbill_model->search_billing();
$this->load->library('excel');
$this->excel->setActiveSheetIndex(0);
//name the worksheet
$this->excel->getActiveSheet()->setTitle('DC Reports');
//set cell A1 content with some text
$this->excel->getActiveSheet()->setCellValue('A1', 'INWARD DETAILS');
$this->excel->getActiveSheet()->setCellValue('A2', 'DATE');
$this->excel->getActiveSheet()->setCellValue('B2', 'DC NO');
$this->excel->getActiveSheet()->setCellValue('C2', 'DC TYPE');
$this->excel->getActiveSheet()->setCellValue('D2', 'CUSTOMER NAME');
$this->excel->getActiveSheet()->setCellValue('E2', 'INWARD NO');
...
$filename='DC Reports-'.date('d/m/y').'.xls'; //save our workbook as this file name
...
$id=$rows['inwardid'];
	...
	if($dctype=='Against Inward')
	{
		...
		$newid = $this->db->select('insertid')->where('id',$id)->get('inward_delivery')->row()->insertid;
		// $newid = $this->db->select('')->where('')->get('')->row();
...
<td><input class="form-control" parsley-trigger="change" required id="" readonly type="text" name="heatno[]" value="'.$rows['heatno'].'"><div id="heatno_valid"></div></td>

<td><input class="form-control" parsley-trigger="change" required id="" readonly type="text" name="itemname[]" value="'.htmlspecialchars($itemid->itemname).'"><div id="itemname_valid"></div><input type="hidden" name="itemid[]" id="" value="'.$itemid->id.'"><input type="text" name="item_desc[]" value="'.$rows['item_desc'].'" style="margin-top: 2px;" class="form-control"><div id="itemname_valid"></div></td>

<td><input class="form-control" value="'.$rows['uom'].'" readonly id="uom" type="text" name="uom[]"  autocomplete="off"><div id="qty_valid"></div></td>

<td><input class="form-control" value="'.$rows['grade'].'" readonly id="grade" type="text" name="grade[]"  autocomplete="off"><div id="qty_valid"></div></td>
...
<input type="text"  class="form-control" name="approximate_value" id="approximate_value" value="'.$rows['approximate_value'].'" style="width:250px;">	
</div>
<br>
...
foreach ($data as $rows) {
	 $itemid=$this->db->where('id',$rows['itemid'])->get('additem')->row();
$approxvalue = $this->db->select('approximate_value')->where('id',$id)->get('dcbill_details')->row()->approximate_value;
...
<input class="form-control" parsley-trigger="change" readonly id="itemid" type="hidden" name="itemid[]" value="'.$rows['itemid'].'"> 
<div id="hsnno_valid"></div>
</td>

<td><input class="form-control" parsley-trigger="change" readonly id="heatno" type="text" name="heatno[]" value="'.$rows['heatno'].'"></td>


<td>
<input class="form-control" parsley-trigger="change" required id="" readonly type="text" name="itemname[]" value="'.htmlspecialchars($itemid->itemname).'">
...
<td>
<input class="form-control" value="'.$rows['grade'].'" readonly id="grade" type="text" name="grade[]" autocomplete="off">
<div id="grade_valid"></div>
</td>
<td>
...
<div class="col-sm-offset-8">
 <div class="col-md-4">
                          <label>Approximate Value</label>
                           ...
                           <div class="col-md-6">
                           <input type="text" class="form-control" name="approximate_value" id="approximate_value" onkeypress="return isNumberKey(event)" value="'.$approxvalue.'" style="width:148px;">
                           </div>
</div>
