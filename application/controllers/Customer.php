...
'rcbio_todate' => $todate,
'rcbio_customername' => $partytype,
);
$this->session->set_userdata($data);
}
public function excel_download(){
    
    $val=$this->customer_model->search_customer();
$this->load->library('excel');
$this->excel->setActiveSheetIndex(0);
//name the worksheet
$this->excel->getActiveSheet()->setTitle('Party Details');
//set cell A1 content with some text
$this->excel->getActiveSheet()->setCellValue('A1', 'Party DETAILS');
...
$this->excel->getActiveSheet()->setCellValue('F2', 'STATE');
...
$data['dcno']=$row['type'];
$data['dctype']=$row['name'];
$data['cusname']=$row['phoneno'];
$data['inwardno']=$row['address1'];
$data['hsnno']=$row['state'];
$data['itemname']=$row['city'];
...
$filename='Party Details-'.date('d/m/y').'.xls'; //save our workbook as this file name
	...
	public function update()
	{
		...
		if($_POST['name']!=$_POST['oldName'])
		{
			...
			if($_POST['type']=='Intra customer' || $_POST['type']=='Inter customer')
			{
				...
				$this->db->query("UPDATE `purchaseorder_details` SET customername='".$_POST['name']."' WHERE customername='".$_POST['oldName']."' ");
				$this->db->query("UPDATE `purchaseorder_reports` SET customername='".$_POST['name']."' WHERE customername='".$_POST['oldName']."' ");
			}
