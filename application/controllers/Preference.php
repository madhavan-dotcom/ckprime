<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Preference extends CI_Controller {
		...
		public function index()
		{
			...
			//$data['cardArray'] = $this->db->get('card')->result_array();
			//print_r($data['cardArray']);
			//exit;
			//print_r($data['row']);
			//exit;
		...
		public function insert()
		{
			...
			'expensePrefix' =>@$_POST['expensePrefix'],
			'expenses' =>@$_POST['expenses'],
			'dc' =>@$_POST['dc'],
			'materialdc' =>$_POST['materialdc'],
			'jobworkdc' =>$_POST['jobworkdc'],
			'voucherPrefix' =>@$_POST['voucherPrefix'],
			'voucher' =>$_POST['voucher'],
			'debitPrefix' =>@$_POST['debitPrefix'],
			'debit' =>$_POST['debit'],
			'creditPrefix' =>@$_POST['creditPrefix'],
			'credit' =>$_POST['credit'],
			'purchasePrefix'=>@$_POST['purchasePrefix'],
			'purchase' =>$_POST['purchase'],
			'invoicePrefix' =>@$_POST['invoicePrefix'],
			'invoice' =>$_POST['invoice'],
			'salesdcPrefix' =>@$_POST['salesdcPrefix'],
			'materialdcPrefix' =>@$_POST['materialdcPrefix'],
			'jobdcPrefix' =>$_POST['jobdcPrefix'],
			'proforma_invoicePrefix' =>@$_POST['proforma_invoicePrefix'],
			'proforma_invoice' =>$_POST['proforma_invoice'],
			'cashbillPrefix' =>$_POST['cashbillPrefix'],
			'cashbill_invoice' =>$_POST['cashbill_invoice'],
			'inwardPrefix' =>$_POST['inwardPrefix'],
			'inward'	=> $_POST['inward'],
			...
			'cont_logo' =>$cont_logo,
			'discountBy' => $_POST['discountBy'],
			'itemType' => $_POST['itemType'],
			'invoiceBy'	=> $_POST['invoiceBy'],
			'bill_type' => $_POST['bill_type'],
			'itemcode' => $itemcode,
			'priceType' => $priceType,
			'quot_bill_type' => $_POST['quot_bill_type'],
			'voucher_receivable' => $_POST['voucher_receivable'],
			'voucher_payable'	=> $_POST['voucher_payable'],
			'cashbill_add'	=> $_POST['cashbill_add'],
			'cashbill_tax_type'=>$_POST['cashbill_tax_type'],
			'inward_add'	=> $_POST['inward_add'],
			'sales_dc'	    => $_POST['sales_dc'],
			'material_dc'	=> $_POST['material_dc'],
			'jobwork_dc'	=> $_POST['jobwork_dc'],
			'cpo_type'      => $_POST['cpo_type'],
			'spo_type'      => $_POST['spo_type'],
			'attendance'    => $_POST['attendance'],
			'einvoice'      => $_POST['einvoice'],
			'eway'          => $_POST['eway']
			);
			// print_r($d);
			// exit;
		...
		public function resetTables()
		{
			...
			foreach($checkedBoxes as $cb)
			{
				...
				{
					//TRUNCATE THE FOLLOWING TABLES
					$this->db->query("TRUNCATE TABLE `inward_details`");
					$this->db->query("TRUNCATE TABLE `inward_delivery`");
				}
				elseif($cb=='stock')
				{
					//TRUNCATE THE FOLLOWING TABLES
				...
				elseif($cb=='proforma_invoice_details')
				{
					//TRUNCATE THE FOLLOWING TABLES
					$this->db->query("TRUNCATE TABLE `proforma_invoice_details`");
					$this->db->query("TRUNCATE TABLE `proforma_invoice_reports`");
				}
				elseif($cb=='purchaseorder_details')
				{
					//TRUNCATE THE FOLLOWING TABLES
					$this->db->query("TRUNCATE TABLE `purchaseorder_details`");
					$this->db->query("TRUNCATE TABLE `purchaseorder_reports`");
				}
				elseif($cb=='sup_purchaseorder_details')
				{
					//TRUNCATE THE FOLLOWING TABLES
					$this->db->query("TRUNCATE TABLE `sup_purchaseorder_details`");
					$this->db->query("TRUNCATE TABLE `sup_purchaseorder_reports`");
				}
				elseif($cb=='sales_return')
				{
					//TRUNCATE THE FOLLOWING TABLES
					$this->db->query("TRUNCATE TABLE `sales_return`");
				}
				elseif($cb=='cashBill')
				{
					//TRUNCATE THE FOLLOWING TABLES
					$this->db->query("TRUNCATE TABLE `cashbill_details`");
					$this->db->query("TRUNCATE TABLE `cashbill_reports`");
				}
				elseif($cb=='full')
				{
					//TRUNCATE THE FOLLOWING TABLES
