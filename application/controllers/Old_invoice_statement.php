...
ob_start();
error_reporting(0);
class Old_invoice_statement extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model('old_invstatement_model');
	...
	public function index()
	{
        ...
        $data['view']=$this->old_invstatement_model->select();
	...
	public function view()
	{
		$data['view']=$this->old_invstatement_model->select();
		$this->load->view('header');
		$this->load->view('old_invoice_stmtlist',$data);
...
//$cusname='ram';
  ...
  {
    $list = $this->old_invstatement_model->get_datatables();
    $data = array();
    $no = $_POST['start'];
    $a=1;
     $totalamount[]=array();
    ...
    $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->old_invstatement_model->count_all(),
            "recordsFiltered" => $this->old_invstatement_model->count_filtered(),
            "data" => $data,
        );
	...
	{   
		...
		if($bill_format=='Print')
		{
			...
			if($data['fromdate']=='' && $data['todate']=='' && $data['customername']=='' && $data['invoiceno']=='')
			{
				...
				$this->load->view('oldinvoice_overall_reports',$data);
			}
			else
			{
				$data['purchase']=$this->old_invstatement_model->search_billing();	
				$this->load->view('oldinvoice_satement_reports',$data);
			}
		...
		elseif($bill_format=='excel_Download')
		{
			...
			$this->excel->getActiveSheet()->setTitle('Sales Party Statement');
			$this->excel->getActiveSheet()->setCellValue('A1', 'SALES PARTY STATEMENT');
			...
			if($data['fromdate']=='' && $data['todate']=='' && $data['customername']=='' && $data['invoiceno']=='')
			{
				$custId_query=$this->db->query("SELECT customerId,customername FROM invoice_details_backup WHERE invoicetype='Direct Invoice' GROUP BY customerId");
				$this->excel->getActiveSheet()->setCellValue('A2', 'Name');
				$this->excel->getActiveSheet()->setCellValue('B2', 'Invoice Amount');
				...
				if($custId_query->num_rows() > 0 )
				{
					...
					foreach($custIdRes as $cR)
					{
						$i++;
						//GET INVOICE AMOUNT	
						$grandInvoiceAmt = $this->db->query("SELECT SUM(`grandtotal`) AS invoiceAmt FROM `invoice_details_backup` WHERE `customerId`='".$cR->customerId."' AND invoicetype='Direct Invoice' ");
						...
						//GET RETURN AMOUNT
						$grandRetAmt = $this->db->query("SELECT SUM(`grandtotal`) AS returnAmt FROM `sales_return_backup` WHERE `customerid`='".$cR->customerId."' ");
						...
						//GET RECEIPT AMOUNT
						$grandReceiptAmt = $this->db->query("SELECT SUM(`voucheramount`) AS receiptAmt FROM `voucher_backup` WHERE vouchertype='Receivable' AND paymentTo = 'Customer' AND `cus_suppId`='".$cR->customerId."' ");
						...
						$data1['suppliername']=$cR->customername;
			...
			else
			{
				...
				if($this->session->userdata('rcbio_customername')!="") 	{ $head_supplier = $this->session->userdata('rcbio_customername'); 	} else { $head_supplier='-'; 	}
				...
				@$suppliername=$this->session->userdata('rcbio_customername');
				$topSuppliername = $suppliername;
				if($suppliername)
				{
					$openingbalance=$this->db->select('old_openingBal')->where('name',$suppliername)->where("(type = 'Inter customer' OR type = 'Intra customer')")->get('customer_details')->row()->old_openingBal;
					//echo $openingbalance;
					//exit;
				...
				$purchase=$this->old_invstatement_model->search_billing();
				//print_r($purchase);
				//exit;
				...
				foreach ($purchase as $row) {
					...
					if($topSuppliername=='')
					{
						$openingbalance=$this->db->select('old_openingBal')->where('name',$row['suppliername'])->where("(type = 'Inter customer' OR type = 'Intra customer')")->get('customer_details')->row()->old_openingBal;
