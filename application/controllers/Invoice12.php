...
for ($i=0; $i <  $count; $i++) 
{ 
$data=$this->db->where('itemname',$sparename[$i])->where('itemcode',$itemcodess[$i])->where('hsnno',$hsnnos[$i])->get('stock')->result();
foreach($data as $v)
{
$bal=$v->balance;
}
if($data)
{
$this->db->where('itemname',$sparename[$i])->where('itemcode',$itemcodess[$i])->where('hsnno',$hsnnos[$i])->update('stock',array('date'=>date('Y-m-d'),'balance'=>$bal-$qty[$i]));
}
}
...
@$discountamount=implode('||',$_POST['discountamount']);
$taxableamount=implode('||',$_POST['taxableamount']);
$othercharges=implode('||',$_POST['ohercharges']);
$subtotal=$this->input->post('subtotal');
$sgst=$this->input->post('sgst');
$sgstamount=$this->input->post('sgstamount');
$cgst=$this->input->post('cgst');
...
'taxableamount'=>$taxableamount,
'sgst'=>implode('||',$sgst),
'sgstamount'=>implode('||',$sgstamount),
'cgst'=>implode('||',$cgst),
'cgstamount'=>implode('||',$cgstamount),
'igst'=>implode('||',$igst),
'igstamount'=>implode('||',$igstamount),
'subtotal' =>$_POST['subtotal'], 
'freightamount'=>$freightamount,
'loadingamount'=>$loadingamount,
'roundOff' =>$roundOff,
...
if($data)
{
$this->db->where('itemname',$sparename[$i])->where('itemcode',$itemcodess[$i])->where('hsnno',$hsnnos[$i])->update('stock',
array(
'date'=>date('Y-m-d'),
'balance'=>$bal-$qty[$i]
));
...
for ($i=0; $i <$counts ; $i++) { 
 $balqty=$this->db->select('balaceqty')->where('id',$deliveryid[$i])->get('purchaseorder_reports')->row()->balaceqty;


$podata=array('balaceqty'=>$qtyss[$i]+$balqty);
$this->db->where('id',$deliveryid[$i]);
$this->db->update('purchaseorder_reports',$podata);

} 

$podatas=array('edit_status'=>1);
