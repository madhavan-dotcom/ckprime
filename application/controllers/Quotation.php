...
class Quotation extends CI_Controller {
  ...
  public function __construct()
  {
	...
	public function index()
	{
		...
		$this->db->limit(1);
		$result=$this->db->get('quotation_details')->result_array();
		...
		else
		{
			...
			else
			{
				...
				else
				{
					$bondLen = strlen($bond_no)-1;
					$bondOnlyNum = filter_var($bond_no, FILTER_SANITIZE_NUMBER_INT);
					$new_bond_noo = 'Q'.sprintf('%0'.$bondLen.'d', (float)$bondOnlyNum + 1);
				}
		...
		if($quotation_tax->quot_bill_type=="Overall Tax"){
                		...
                		$this->load->view('add_quotation_overalltax',$data);
	...
	//	$description=implode('||',$_POST['description']);
	 ...
	 $sales_bill_type=$this->db->where('id', '1')->get('preference_settings')->row()->quot_bill_type;
if($sales_bill_type=='Overall Tax'){
	...
	public function views()
	{
		...
		if($quotation_tax->quot_bill_type=="Overall Tax"){
		    		$this->load->view('header');
            		$this->load->view('quotation_overalltax_view',$data);
            		$this->load->view('footer');
		}elseif($quotation_tax->quot_bill_type=="Multiple Tax"){
		    		$this->load->view('header');
...
// $this->load->view('invoice_bill',$data);
  ...
  // $this->load->library('mpdf'); 
    ...
    $data['invoice']=$this->db->where('id',$id)->get('quotation_details')->result();
     ...
     $quotation_tax=$this->db->where('id',1)->get('preference_settings')->row();
		if($quotation_tax->quot_bill_type=="Overall Tax"){
		    	 $this->load->view('quotationbill_overalltax',$data);
		}elseif($quotation_tax->quot_bill_type=="Multiple Tax"){
		    		 $this->load->view('quotationbill',$data);
		}
...
public function autocomplete_hsnno()
{
  ...
  echo json_encode($name);
	...
	public function edit()
	{
		...
		if($quotation_tax->quot_bill_type=="Overall Tax"){
		    		$this->load->view('header');
            		$this->load->view('quotation_overalltax_edit',$data);
            		$this->load->view('footer');
		}elseif($quotation_tax->quot_bill_type=="Multiple Tax"){
		    			$this->load->view('header');
