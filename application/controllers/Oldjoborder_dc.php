...
ob_start();
error_reporting(0);
class Oldjoborder_dc extends CI_Controller {
public function __construct()
{
parent::__construct();
$this->load->model('oldjoborderdc_model');
...
$data['pre']=$this->db->where('id',$id)->get('jobworkdc_details_backup')->result();
$this->load->view('dcbills',$data);
}

public function view()
{
$data['dcbill']=$this->oldjoborderdc_model->select();
$this->load->view('header');
$this->load->view('oldjoborderdc_view',$data);
...
{
$list = $this->oldjoborderdc_model->get_datatables();
$data = array();
$no = $_POST['start'];
$a=1;
$totalamount[]=array();
...
<li><a class="" target="_blank" style="color:white; font-weight: bold;background-color: #23BDCF;" href="'.base_url().'oldjoborder_dc/rebill/'.$person->id.'" title="Print" >Print</a></li>
';
...
$output = array(
"draw" => $_POST['draw'],
"recordsTotal" => $this->oldjoborderdc_model->count_all(),
"recordsFiltered" => $this->oldjoborderdc_model->count_filtered(),
"data" => $data,
);
...
{
$data['purchase']=$this->oldjoborderdc_model->search_billing();
