...
$data=$this->db->get('profile')->result();
					...
					<div class="col-sm-12">
						<div class="portlet box blue">
							<div class="portlet-title">
								<div class="caption">
									<i class="fa fa-reorder"></i>Inward Stock Reports
								</div>
								<div class="tools">
							...
							<div class="portlet-body form">
								...
								<form class="horizontal-form" id="form" method="post" action="<?php echo base_url();?>inward_delivery/insert">
									<div class="form-body">
										<div class="row">
											<div class="col-md-3">
								...
								<!-- START HERE-->
								<form method="post" id="form-filter" action="<?php echo base_url();?>inwardstock/search">
								  <table>

								  <td style="width: 120px;">Customer Name</td>
									<td><input type="text" class="form-control" name="customername" id="scustomername" style="font-size:16px;width:145px;" ></td>
									<td style="width: 120px;">&nbsp;&nbsp;&nbsp;Item Name</td>
									<td><input type="text" class="form-control" name="itemname" id="sitemname" style="font-size:16px;width:143px;" ></td>
						...
						<!--			  <th>Rate</th>   -->
								  ...
								  <form name="form" method="post" action="<?php echo base_url();?>inward/reports" target="_blank" >
									<table>
									  ...
									  <td><input type="hidden" name="dcno" id="dcno" class="form-control" value="<?php if($this->input->post('dcno')){echo $this->input->post('dcno');}?>"></td>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			"ajax": {
				"url": "<?php echo site_url('inwardstock/ajax_list')?>",
				"type": "POST",
				"data": function ( data ) {
				
					data.fromdate = $('#sfromdate').val();
					data.todate = $('#stodate').val();
					data.customername = $('#scustomername').val();
					data.itemname = $('#sitemname').val();
				}
			},
			...
			$('#btn-filter').click(function(){ //button filter event click
				$("#pcustomername").val($('#scustomername').val());
		
				$('#pitemname').val($('#sitemname').val());
		...
		*/
		...
		$( "#sitemname" ).autocomplete({
			source: function(request, response) {
				$.ajax({ 
					url: "<?php echo base_url();?>inwardstock/autocomplete_itemname",
		...
		$( "#scustomername" ).autocomplete({
			source: function(request, response) {
				$.ajax({ 
					url: "<?php echo base_url();?>inwardstock/autocomplete_customername",
					...
					success: function(data){              
						...
						if(data=='')
						{
							$('#scustomername').focus();
							$('#scustomername').val('');
							$('#hsnno_valid').html('<div><font color="red">Invalid customer code</font></div>');
							$('#scustomername').keyup(function(){
								$('#hsnno_valid').html('');
							});
							return false
						}
		...
		$('#itemname').blur(function(){
			var itemname=$('#itemname').val();
			$.post('<?php echo base_url();?>inward_delivery/get_itemname',{itemname:itemname},function(res){    
		...
		$('#hsnno').blur(function(){
			...
			$.post('<?php echo base_url();?>inward_delivery/get_itemcode',{itemcode:itemcode},function(res){   
				var obj=jQuery.parseJSON(res);
